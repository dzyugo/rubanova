...
function CartPage() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-6 py-12 md:py-16">
      <h1 className="font-display text-5xl font-bold text-primary md:text-6xl">{t("cart.title")}</h1>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_380px]">
            ...
            {items.map((i) => (
              <article key={i.slug} className="flex gap-4 rounded-2xl bg-card p-4 shadow-sm">
                ...
                <div className="flex flex-col items-end justify-between">
                  <span className="font-display text-lg font-bold">{(i.price * i.qty).toFixed(2)} DA</span>
          ...
          <aside className="h-fit rounded-3xl bg-tertiary/50 p-6">
            ...
            <dl className="mt-6 space-y-3 text-sm">
              <div className="flex justify-between"><dt>{t("cart.subtotal")}</dt><dd className="font-semibold">{subtotal.toFixed(2)} DA</dd></div>
              <div className="flex justify-between"><dt>{t("cart.shipping")}</dt><dd className="font-semibold">{shipping.toFixed(2)} DA</dd></div>
            </dl>
            <div className="mt-5 flex items-baseline justify-between border-t border-border pt-5">
              <span className="font-display text-xl font-bold">{t("checkout.total")}</span>
              <span className="font-display text-3xl font-bold text-primary">{total.toFixed(2)} DA</span>
            </div>
            ...
            <div className="mt-6 flex gap-3 border-t border-border pt-5">
              ...
              <div>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t("cart.sustainable")}</p>
              </div>
            </div>
          </aside>
        </div>
